...
require_once($CFG->dirroot . "/blocks/myoverview/renderer.php");
require_once($CFG->dirroot . "/lib/ortutils.php");

class theme_eguru_block_myoverview_renderer extends block_myoverview_renderer {

    /**
     * Construct contents of myoverview block
     *
     * @param array $courses list of courses in sorted order
     * @param array $overviews list of course overviews
     * @return string html to be displayed in myoverview block
     */
    public function myoverview($courses, $overviews) {
        $html = '';
        $config = get_config('block_myoverview');
        if ($config->showcategories != BLOCKS_MYOVERVIEW_SHOWCATEGORIES_NONE) {
            global $CFG;
        ...
        if ($this->page->user_is_editing() && (count($courses) > 1)) {
            $userediting = true;
            $this->page->requires->js_init_call('M.block_myoverview.add_handles');
        ...
        if ($ismovingcourse) {
            ...
            $html .= get_string('movingcourse', 'block_myoverview', $a);
            $html .= $this->output->box_end();

            $moveurl = new moodle_url('/blocks/myoverview/move.php',
            ...
            $movetofirsticon = html_writer::empty_tag('img',
                array('src' => $this->output->image_url('movehere'),
                    'alt' => get_string('movetofirst', 'block_myoverview', $courses[$movingcourseid]->fullname),
        ...
        foreach ($courses as $key => $course) {
            ...
            if ($userediting && !$ismovingcourse) {
                $moveicon = html_writer::empty_tag('img',
                    array('src' => $this->image_url('t/move')->out(false),
                        'alt' => get_string('movecourse', 'block_myoverview', $course->fullname),
            ...
            if ($course->id > 0) {
                ...
                $html .= $this->display_myoverviewfiles($course->id);
            } else {
            ...
            if (!empty($config->showchildren) && ($course->id > 0)) {
                // List children here.
                if ($children = block_myoverview_get_child_shortnames($course->id)) {
            ...
            if ($config->showcategories != BLOCKS_MYOVERVIEW_SHOWCATEGORIES_NONE) {
                ...
                if ($currentcategory !== null) {
                    ...
                    if ($config->showcategories == BLOCKS_MYOVERVIEW_SHOWCATEGORIES_FULL_PATH) {
            ...
            if ($ismovingcourse) {
                ...
                $movehereicon = html_writer::empty_tag('img',
                    array('src' => $this->output->image_url('movehere'),
                        'alt' => get_string('moveafterhere', 'block_myoverview', $a),
                        'title' => get_string('movehere')));
     ...
     * @return bool
     */
    public function has_myoverviewfiles($courseid) {
        global $CFG;
     ...
     */
    public function get_myoverviewfiles($courseid) {
        global $CFG;
        if (empty($CFG->courseoverviewfileslimit)) {
            return array();
        }
        ...
        if (count($files)) {
            $overviewfilesoptions = myoverviewfiles_options($courseid);
    ...
    protected function display_myoverviewfiles($courseid) {
        global $CFG;
        // display course overview files
        $contentimages = $contentfiles = '';
        foreach ($this->get_myoverviewfiles($courseid) as $file) {
