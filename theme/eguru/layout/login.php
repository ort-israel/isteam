...
<?php require_once(dirname(__FILE__) . '/includes/body_classes.php'); ?>
    ...
    <div class="container-fluid">
        ...
        <div class="loginbox">
            ...
            <div class="formwrap">
                <form action="<?php echo $CFG->httpswwwroot; ?>/login/index.php" method="post" id="login1">
                    ...
                    <div class="support-fields">
                        <label class="checkbox"><input
                                type="checkbox"> <?php print_string('rememberusername', 'admin') ?>
                        </label>
                ...
                <?php
                /* Lea 2016/03 -
                Added false to the condition, so the button is never displayed.
                This is do to the fact that $CFG->guestloginbutton has to be true in order that
                "automatic guest login" will be enabled, but it has to be false, so the button isn't shown.
                The reason is shouldn't be shown is that since any course can be accessed just by going to its url, there is no need for a
                guest login button isn't needed at all in the site.*/
                if (false && $CFG->guestloginbutton and !isguestuser()) { ?>
